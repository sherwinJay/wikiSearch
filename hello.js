$(document).ready(function() {
	...
	function randomQuote(){
		$("form").on("keyup", "input", function(){
			...
			$.ajax({
			   url: "https://en.wikipedia.org/w/api.php?action=query&prop=revisions&rvprop=content&list=allpages&apfrom=" + inputVal.charAt(0) + "&apto=" + inputVal.charAt(-1) + "&apprefix="+ inputVal + "&apfilterrendir=all&format=json",
			   type: "POST",
			   dataType: "jsonp",
			   header: {"Api-User-Agent" : "wikiSearch"},
			   success: function(data){
				console.log("https://en.wikipedia.org/w/api.php?action=query&prop=revisions&rvprop=content&list=allpages&apfrom=" + inputVal);
